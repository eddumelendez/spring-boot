...
public class SpringMeters {
	 ...
	 * look up information about pool usage.
	 * @param name The name prefix of the metrics.
	 * @param tags Tags to apply to all recorded metrics.
	 * @return The instrumented data source, unchanged. The original data source is not
	...
	public static DataSource monitor(MeterRegistry registry, DataSource dataSource,
			Collection<DataSourcePoolMetadataProvider> metadataProviders, String name,
			Tag... tags) {
		return monitor(registry, dataSource, metadataProviders, name, Arrays.asList(tags));
	}
	 ...
	 */
	public static ThreadPoolTaskExecutor monitor(MeterRegistry registry,
			ThreadPoolTaskExecutor executor, String name, Iterable<Tag> tags) {
	 ...
	 */
	public static ThreadPoolTaskExecutor monitor(MeterRegistry registry,
			ThreadPoolTaskExecutor executor, String name, Tag... tags) {
		Meters.monitor(registry, executor.getThreadPoolExecutor(), name, tags);
		return executor;
	}
	 ...
	 */
	public static ThreadPoolTaskScheduler monitor(MeterRegistry registry,
			ThreadPoolTaskScheduler scheduler, String name, Iterable<Tag> tags) {
	 ...
	 */
	public static ThreadPoolTaskScheduler monitor(MeterRegistry registry,
			ThreadPoolTaskScheduler scheduler, String name, Tag... tags) {
		Meters.monitor(registry, scheduler.getScheduledExecutor(), name, tags);
		return scheduler;
	}
}
